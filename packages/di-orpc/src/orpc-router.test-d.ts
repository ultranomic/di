...
import type { InjectableClass, InjectEntry } from "@ultranomic/di";
...
class UserRouter extends OrpcRouter({
  ...
  getById = this.orpc.handler(async ({ input }: { input: { id: string } }) => input.id);
}
...
describe("OrpcRouter types", () => {
  ...
  test("static _inject is correct type", () => {
    ...
    assertType<readonly []>(EmptyRouter._inject);
  });
  ...
  test("instance has orpc builder", () => {
    ...
    assertType<object>(instance.orpc);
  });
