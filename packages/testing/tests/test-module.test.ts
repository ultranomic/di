...
import { Test, TestingModule, TestModuleBuilder } from '../src/test-module.js'
import { Module, Container, type ModuleMetadata } from '@voxeljs/core'
...
describe('TestModuleBuilder', () => {
  describe('compile', () => {
    ...
    it('should compile with providers with dependencies', async () => {
      ...
      class UserService {
        ...
        getUsers() {
          this.deps.logger.log('Getting users')
          return ['user1']
        }
      }
  ...
  describe('addProvider', () => {
    it('should add a provider to the test module', async () => {
      ...
      const testingModule = await Test.createModule()
        .addProvider('MyService', mockService)
        .compile()
