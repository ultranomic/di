 ...
 */
export type InferRoutesFromApp<T> = T extends Hono<infer E, infer S>
  ? S
  : never
 ...
 * ```
 */
export function createRpcClient<T extends { getApp(): Hono }>(
  adapter: T,
  baseUrl: string
) {
 ...
 * ```
 */
export function createClientFromApp<T extends Hono>(
  app: T,
  baseUrl: string
): ReturnType<typeof hc<typeof app>> {
  return hc<typeof app>(baseUrl)
}
 ...
 */
export type RpcClient<T extends { getApp(): Hono }> = ReturnType<
  typeof createRpcClient<T>
>
