...
import { BindingScope } from '../../src/container/binding.js'
import { ScopeValidationError } from '../../src/errors/scope-validation.js'
...
describe('Container Scope Support', () => {
  ...
  describe('scoped caching', () => {
    it('should cache scoped services per scope', () => {
      let instanceCount = 0
      container
        .register('ScopedService', () => ({ id: ++instanceCount }))
        .asScoped()
  ...
  describe('singleton across scopes', () => {
    it('singleton should be shared across all scopes', () => {
      let instanceCount = 0
      container
        .register('SingletonService', () => ({ id: ++instanceCount }))
        .asSingleton()
  ...
  describe('scope validation', () => {
    ...
    it('should throw when called on child container', () => {
      ...
      expect(() => scope.validateScopes()).toThrow(
        /validateScopes\(\) must be called on the root container/
      )
    })
  })
