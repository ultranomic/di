...
import type { InferDeps, InjectableClass } from '../../src/types/deps.js'
  ...
  describe('class with static inject', () => {
    ...
    it('should resolve class with single dependency', () => {
      ...
      class UserService {
        ...
        constructor(
          private deps: InferDeps<typeof UserService.inject, { Logger: Logger }>,
        ) {}
    ...
    it('should resolve nested dependencies', () => {
      ...
      class Database {
        static readonly inject = { logger: 'Logger' } as const
        constructor(
          private deps: InferDeps<typeof Database.inject, { Logger: Logger }>,
        ) {}
        query(sql: string) {
      ...
      class UserService {
        ...
        constructor(
          private deps: InferDeps<typeof UserService.inject, { Database: Database }>,
        ) {}
        getUser(id: string) {
      ...
      container.register('Database', (c) => {
        const deps = c.buildDeps(Database.inject) as InferDeps<
          typeof Database.inject,
          { Logger: Logger }
        >
  ...
  describe('InjectableClass type', () => {
    it('should work with InjectableClass type for type-safe registration', () => {
      ...
      class MyService {
        static readonly inject = { logger: 'Logger' } as const
        constructor(
          private deps: InferDeps<typeof MyService.inject, { Logger: Logger }>,
        ) {}
        doSomething() {
      ...
      const _ServiceClass: InjectableClass<
        typeof MyService.inject,
        MyService
      > = MyService

      container.register('Logger', () => ({ log: (_msg: string) => {} }))
      container.register('MyService', (c) => {
        const deps = c.buildDeps(_ServiceClass.inject) as InferDeps<
          typeof _ServiceClass.inject,
          { Logger: Logger }
        >
  ...
  describe('resolution context tracking', () => {
    ...
    it('should track resolution path for nested resolution failures', () => {
      class Level1Service {
        ...
        constructor(
          _deps: InferDeps<typeof Level1Service.inject>,
        ) {}
      }
  ...
  describe('singleton scope with deps', () => {
    it('should reuse singleton dependencies', () => {
      ...
      class ServiceA {
        static readonly inject = { logger: 'Logger' } as const
        constructor(
          private deps: InferDeps<typeof ServiceA.inject, { Logger: Logger }>,
        ) {}
      }
      ...
      class ServiceB {
        static readonly inject = { logger: 'Logger' } as const
        constructor(
          private deps: InferDeps<typeof ServiceB.inject, { Logger: Logger }>,
        ) {}
      }
      ...
      container.register('ServiceA', (c) => {
        const deps = c.buildDeps(ServiceA.inject) as InferDeps<
          typeof ServiceA.inject,
          { Logger: Logger }
        >
      ...
      container.register('ServiceB', (c) => {
        const deps = c.buildDeps(ServiceB.inject) as InferDeps<
          typeof ServiceB.inject,
          { Logger: Logger }
        >
