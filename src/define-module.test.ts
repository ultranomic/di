...
import { defineModule, type Module } from './define-module.ts';
import { type Service } from './define-service.ts';

describe('defineModule', () => {
  describe('handler without dependencies', () => {
    it('should create module returning object', () => {
    ...
    it('should create module returning void', () => {
      const module = defineModule.handler(() => {
    ...
    it('should group related functionality', () => {
      const authModule = defineModule.handler(() => ({
  ...
  describe('handler with dependencies', () => {
    it('should create module with service dependencies', () => {
      ...
      const module = userModule(() => mockDeps);
    ...
    it('should support module composition with other modules', () => {
      ...
      const module = contentModule(() => mockDeps);
  ...
  describe('module type constraints', () => {
    it('should enforce Record or void return types', () => {
      ...
      const recordModule = defineModule.handler(() => ({ key: 'value' }));
      const voidModule = defineModule.handler(() => {});
      
      // Verify they work as expected
      assert.deepStrictEqual(recordModule(), { key: 'value' });
      assert.strictEqual(voidModule(), undefined);
    });
    ...
    it('should maintain Module type wrapper', () => {
      ...
      const typedModule: () => Module<{ method: () => string }> = module;
      assert.strictEqual(typedModule().method(), 'result');
    });

    it('should support complex record types', () => {
      const complexModule = defineModule.handler(() => ({
        strings: {
          hello: 'world',
          foo: 'bar'
        },
  ...
  describe('real-world module patterns', () => {
    it('should support feature module pattern', () => {
      const ecommerceModule = defineModule.handler(() => ({
        cart: {
      ...
      const module = ecommerceModule();
      
      // Test cart functionality
      module.cart.addItem('item1', 2, 10.00);
      module.cart.addItem('item2', 1, 15.00);
      
      assert.strictEqual(module.cart.getTotal(), 35.00);
      assert.strictEqual(module.cart.items.length, 2);
      
      // Test checkout functionality
      const order = module.checkout.processOrder(module.cart.items);
      assert.strictEqual(order.subtotal, 35.00);
