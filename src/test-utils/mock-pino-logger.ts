 ...
 */
export const createMockPinoLogger = (name = 'mock'): Logger => {
  const logs: Array<{ level: string; msg: string; prefix?: string }> = [];
  
  const createLogger = (prefix?: string): Logger => ({
    // Standard pino log levels
    trace: (msg: string) => logs.push({ level: 'trace', msg, prefix }),
    debug: (msg: string) => logs.push({ level: 'debug', msg, prefix }),
    info: (msg: string) => logs.push({ level: 'info', msg, prefix }),
    warn: (msg: string) => logs.push({ level: 'warn', msg, prefix }),
    error: (msg: string) => logs.push({ level: 'error', msg, prefix }),
    fatal: (msg: string) => logs.push({ level: 'fatal', msg, prefix }),
