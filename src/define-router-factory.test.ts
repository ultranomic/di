...
/**
 * Test Plan for defineRouterFactory
 *
 * BASIC FUNCTIONALITY:
 * 1. ✅ Should create router factory returning object
 * 2. ✅ Should create router factory returning void
 * 3. ✅ Should support multiple HTTP methods
 *
 * DEPENDENCY INJECTION:
 * 4. ✅ Should create router with service dependencies
 * 5. ✅ Should support module dependencies
 *
 * LIFECYCLE HOOKS:
 * 6. ✅ Should support lifecycle hooks
 *
 * INTEGRATION PATTERNS:
 * 7. ✅ Should support middleware patterns
 *
 * TYPE CONSTRAINTS:
 * 8. ✅ Should enforce Record or void return types
 * 9. ✅ Should maintain Router type wrapper
 *
 * REAL-WORLD PATTERNS:
...
describe('defineRouterFactory', () => {
  ...
  describe('router type constraints', () => {
    it('should enforce Record or void return types', () => {
      ...
      const defineRecordRouter = defineRouterFactory
        .name('RecordRouter')
        .inject()
        .handler(() => ({
          '/route': { GET: () => 'response' },
        }));
      const defineVoidRouter = defineRouterFactory
        .name('VoidRouter')
        .inject()
        .handler(() => {});
  ...
  describe('real-world router patterns', () => {
    it('should support REST API pattern', () => {
      ...
      const defineRestRouter = defineRouterFactory
        .name('RestRouter')
        .inject()
        .handler(() => {
