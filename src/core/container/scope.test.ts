...
describe('Container Scope Support', () => {
  ...
  describe('createScope', () => {
    ...
    it('should not allow registration in child container', () => {
      class NewService {
        static readonly inject = [] as const satisfies DepsTokens<NewService>;
        data = 'test';
      }
      const scope = container.createScope();
  ...
  describe('scoped caching', () => {
    it('should cache scoped services per scope', () => {
      let instanceCount = 0;
      class ScopedService {
        static readonly inject = [] as const satisfies DepsTokens<ScopedService>;
        id: number;
        constructor() {
  ...
  describe('singleton across scopes', () => {
    it('singleton should be shared across all scopes', () => {
      ...
      class SingletonService {
        static readonly inject = [] as const satisfies DepsTokens<SingletonService>;
        id: number;
        constructor() {
          this.id = ++instanceCount;
        }
  ...
  describe('transient across scopes', () => {
    it('transient should always create new instance', () => {
      ...
      class TransientService {
        static readonly inject = [] as const satisfies DepsTokens<TransientService>;
        id: number;
        constructor() {
          this.id = ++instanceCount;
        }
  ...
  describe('mixed scopes', () => {
    it('should handle singleton depending on transient', () => {
      class TransientDep {
        static readonly inject = [] as const satisfies DepsTokens<TransientDep>;
        value: number;
        constructor() {
      ...
      class SingletonService {
        static readonly inject = [TransientDep] as const satisfies DepsTokens<SingletonService>;
        dep: TransientDep;
    ...
    it('should handle scoped depending on singleton', () => {
      class SingletonDep {
        static readonly inject = [] as const satisfies DepsTokens<SingletonDep>;
        value = 'singleton';
      }
      class ScopedService {
        static readonly inject = [SingletonDep] as const satisfies DepsTokens<ScopedService>;
        dep: SingletonDep;
  ...
  describe('scope validation', () => {
    it('should throw when singleton depends on scoped', () => {
      class ScopedDep {
        static readonly inject = [] as const satisfies DepsTokens<ScopedDep>;
        value = 'scoped';
      }
      class SingletonService {
        static readonly inject = [ScopedDep] as const satisfies DepsTokens<SingletonService>;
        dep: ScopedDep;
    ...
    it('should pass when singleton depends on another singleton', () => {
      ...
      class SingletonService {
        static readonly inject = [SingletonDep] as const satisfies DepsTokens<SingletonService>;
        dep: SingletonDep;
        constructor(dep: SingletonDep) {
          this.dep = dep;
        }
  ...
  describe('has and getBinding inheritance', () => {
    it('has should check parent bindings', () => {
      ...
      expect(scope.has(Logger)).toBe(true);
      class NonExistent {
        static readonly inject = [] as const satisfies DepsTokens<NonExistent>;
      }
